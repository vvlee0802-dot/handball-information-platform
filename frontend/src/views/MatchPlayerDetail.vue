  ...
  <div class="player-match-page">
    ...
    <main class="content">
      ...
      <section class="profile-card">
        ...
        <div class="intro-box">介绍：{{ playerName }} 运动员简介</div>
      </section>
        ...
        <table class="detail-table">
          ...
          <tbody>
            <tr v-for="stat in stats" :key="stat.type">
              ...
              <td>{{ stat.rate }}</td>
              <td v-for="shot in stat.shots" :key="shot.id">
                <span v-if="shot.time" class="shot-time">
                  {{ shot.time }}
                  <button class="video-btn" @click="playVideo(shot.videoUrl)">▻</button>
                </span>
              </td>
              <td v-for="index in Math.max(0, 3 - stat.shots.length)" :key="`empty-${stat.type}-${index}`"></td>
            </tr>
...
const playerName = computed(() => String(route.query.player ?? `队员${route.params.playerId}`))

const stats = [
  { type: '远射', rate: '8/10', shots: [{ id: 1, time: '1:05', videoUrl: '#' }] },
  { type: '近射', rate: '', shots: [] },
  { type: '边射', rate: '', shots: [] },
  { type: '七米球', rate: '', shots: [] },
  { type: '快攻', rate: '', shots: [] },
